...
// real .ds-hero pipeline against tests/fixtures/barerhit.ds-hero.json and
// tests/fixtures/barerhit.background-links.json — no live Obsidian vault
// needed, since buildHeroNote/flattenHeroFeatures/computeHeroStats are all
// pure functions. Re-run this (and manually review the output) whenever
// barerhit.ds-hero.json itself changes; otherwise the fixture is committed
// as-is and this script doesn't run as part of `npm test`.
...
const links = JSON.parse(readFileSync(join(ROOT, "tests/fixtures/barerhit.background-links.json"), "utf8"));
...
const note = buildHeroNote(hero, stats, flat, links);
